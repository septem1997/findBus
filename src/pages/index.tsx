...
export default class extends React.Component {
  ...
  onTipsClick = (item: any) => {
    ...
    //根据起、终点坐标查询公交换乘路线
        transfer.search(new this.AMap.LngLat(this.myPosition.lng,this.myPosition.lat), new this.AMap.LngLat(item.location.lng, item.location.lat),
      ...
      <div className={styles.map} id="container">
        <Modal visible={this.state.searchBarDisplay}>
          <div className={styles.modal}>
            <SearchBar onChange={this.onSearchBarChange} className={styles.searchBar} ref={this.searchBar}
                       placeholder={'请输入您的目的地'} onBlur={this.closeSearchModal} onCancel={this.closeSearchModal}/>
            <List>
              {this.state.tipsList && this.state.tipsList.map((item: any) => <Item arrow={'horizontal'}
                                                                                   onClick={this.onTipsClick.bind(this, item)}
